...
Class USER
{
	...
	function main( $GET )
	{
		...
		{
			return $this->DevTools->ThemeMsg( $this->DevTools->lang['pay_error_title'], $this->local_lang['off'] );
		}
	...
	function pay( $DATA )
	{
		...
		{
			exit( $this->model(
				$this->DevTools->lang['pay_error_title'],
				$this->DevTools->lang['pay_need_login']
			));
		}
		...
		{
			exit( $this->model($this->local_lang['replay'], sprintf( $this->local_lang['already'], $Access['payhide_pagelink'] )) );
		}

		return $this->pay_balance( $Get );
	}
