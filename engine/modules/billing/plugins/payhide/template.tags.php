...
if( ! function_exists('BillingPayhideParser') )
{
	...
	function BillingPayhideParser( $Params )
	{
		...
		$SearchPay = $db->super_query( "SELECT * FROM " . USERPREFIX . "_billing_payhide
											WHERE payhide_user='" . $member_id['name'] . "'
													and payhide_tag='" . $Data['key'] . "'
													and	( payhide_post_id = '0' or payhide_post_id = '" . $Data['post_id'] . "' )
													and	payhide_price='" . $Data['price'] . "'
													and	( payhide_time = 0 or payhide_time >= '" . $_TIME . "' )" );
		...
		$_Content = str_replace('{link}', urlencode( $newstr ), $_Content);
